function [Labels]=MainNFCC_Wardat(MuxNetwork,GT_CubeMux,MinNOC,MaxNOC,NOCBy,factorization)
%Input:
...
%Example: [QM_csnmf]=MainNFCC(MuxNetwork,GT_Cube,MinNOC,MaxNOC,'modularity','SNMF');
...
Labels=repmat(Labels_n,size(MuxNetwork.GroundTruth,1)/size(Labels_n,1),1);% In here i replicated the Labels to all layers
% Instead you can comment this line out and use the variable Labels_n
% instead

NFCC_cpuTimes=toc(Tic_NFCC); 

% In the following use the quality metrics you have
