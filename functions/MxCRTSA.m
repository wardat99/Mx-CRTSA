function [Plabel,Timecost,Uf1,U,Z] = MxCRTSA(A,mu,lambda,numClust)
% Discovering Community structure in Multiplex Networks via a Co-regularized 
%       Robust Tensor-based Spectral Approach code.
% Definition:
%     [Plabel,Timecost]=MxCRTSA(A,mu,lambda,gamma1,gamma2,tol,numClust)
%
% Inputs:
...
for i=1:max_iter
        ...
        % update Z
            ...
            Z(:,:,v)=inv(t(:,:,v))*(gamma2*B(:,:,v)-Y2(:,:,v)+ gamma1*A(:,:,v)'*N(:,:,v)...
                +lambda*(D(:,:,v)^(-0.5))*(U(:,:,v)*U(:,:,v)')*(D(:,:,v)^(-0.5)));
            end 
        
            % make sure Z is symmetric
        for v=1:n3
       ...
       % compute Zmod
        for v=1:1:n3
            D1(:,:,v)=getD(Z(:,:,v));
            Zmod(:,:,v)=D1(:,:,v)^(-0.5)*Z(:,:,v)*D1(:,:,v)^(-0.5)+w_v(v)*(Uopt*Uopt');
        end
